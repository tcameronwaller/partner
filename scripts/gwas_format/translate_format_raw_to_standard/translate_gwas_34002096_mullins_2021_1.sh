...
# Maximal value in column "$14" (cases): 41,917
# Maximal value in column "$15" (controls): 371,549
# Maximal value in column "$13" (half effective sample size): 50,981.48

# Determine how many lines to skip.
# $ zcat ./pgc-bip2021-all.vcf.tsv.gz | awk 'BEGIN{FS=" "; OFS=" "} NR>72{print $0}' | head -10

# Determine maximal counts of observations (samples) for cases and controls.
# $ zcat pgc-bip2021-all.vcf.tsv.gz | awk 'BEGIN{FS=" "; OFS=" "; a=0} NR>73{if ((toupper($13) != "NA") && (($13+0)>(a+0))) a=$13} END{print a}'
...
if [ "$fill_observations" == "1" ] && [ "$fill_case_control" == "1" ]; then
  zcat $path_file_source | awk -v observations=$observations -v cases=$cases -v controls=$controls 'BEGIN {FS = " "; OFS = " "} NR > 73 {
    print $3, $1, $2, toupper($4), toupper($5), (($10*0.101)+($11*0.899)), $6, $7, $8, (observations), "NA", $12, (cases), (controls)
  }' >> $path_file_temporary_format
elif [ "$fill_observations" != "1" ] && [ "$fill_case_control" != "1" ]; then
  zcat $path_file_source | awk 'BEGIN {FS = " "; OFS = " "} NR > 73 {
    print $3, $1, $2, toupper($4), toupper($5), (($10*0.101)+($11*0.899)), $6, $7, $8, ($14 + $15), "NA", $12, $14, $15
  }' >> $path_file_temporary_format
fi
