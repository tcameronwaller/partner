...
path_file_genotype_vcf_source=${3} # full path to source genotype file in VCF format
threads=${4} # count of processing threads to use
path_script_split_chromosome=${5} # full path to script to split by chromosome into genotype files in VCF format
...
if true; then
  ...
  /usr/bin/bash "${path_script_split_chromosome}" \
