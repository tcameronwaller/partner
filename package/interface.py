...
def define_main_subparser(subparsers=None):
    ...
    parser_main.add_argument(
        "-format_tables", "--format_tables", dest="format_tables",
        action="store_true",
        help=(
            "Organize format of multiple tables from a directory."
        )
    )
    # Define behavior.
...
def evaluate_main_parameters(arguments):
    ...
    # Execute procedure.
    if arguments.format_tables:
        # Report status.
        print("... executing format_tables procedure ...")
        # Execute procedure.
        format_tables.execute_procedure(
